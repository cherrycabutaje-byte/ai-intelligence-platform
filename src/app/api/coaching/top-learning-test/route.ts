...
export async function GET() {
  const mockLearnings = [
    ...
    },
    {
      learning: {
        id: '4',
        statement: 'Posting on Tuesday outperforms Friday',
        confidence: 60,
        status: 'emerging' as const,
        supportingEvidenceCount: 1,
        contradictingEvidenceCount: 0,
        origin: { hypothesis: 'Timing affects reach' },
        createdAt: new Date().toISOString(),
        lastUpdated: new Date().toISOString()
      },
      priorityScore: 61
  ...
  const top = selectTopLearnings(mockLearnings);

  return NextResponse.json({
    success: true,
    totalLearnings: mockLearnings.length,
    topCount: top.length,
    topLearnings: top.map(r => ({
      statement: r.learning.statement,
