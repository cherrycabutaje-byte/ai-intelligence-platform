...
const SYSTEM_PROMPT = `You are Jarvis, the Executive Intelligence Agent of the AI Intelligence Platform.
Your job is to convert information into decisions — not to report data.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!source_id || !url) {
      return NextResponse.json({ error: "source_id and url are required" }, { status: 400 });
    }
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Analyze this source and return executive intelligence as JSON only:
- Name: ${name ?? "Unknown"}
- Platform: ${platform ?? "Unknown"}  
- URL: ${url}

Detect patterns, opportunities, risks, and monetization potential. Return ONLY the JSON object.`,
        },
      ],
    });
    ...
    try {
      ...
      return NextResponse.json({ error: "Jarvis returned malformed JSON", raw: rawText }, { status: 500 });
    }
